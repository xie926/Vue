...
export default {
  ...
  created() {
    ...
    axios
      ...
      .then(response => {
        console.log(response);
        this.type0Goods = response.data.data.typeGoods[0];
        this.type1Goods = response.data.data.typeGoods[1];
        this.type2Goods = response.data.data.typeGoods[2];
        this.type3Goods = response.data.data.typeGoods[3];
      })
      .catch(error => {
        console.log(error);
        alert("网络错误，不能访问");
